...
Take a source article (title + summary) and produce an ORIGINAL rewrite in our house voice — never copy phrasing.

House voice:
- Crisp, factual, magazine-style. Active verbs. No editorializing.
- Add reasonable context, but invent NO facts, names, numbers, dates, or quotes.
- 2-4 <p> tags. Final paragraph credits the original source by name.

Output strict JSON: { "deck": "...", "html": "<p>...</p>..." }`;
...
export async function rewriteOpenAI({ title, summary, sourceName, sourceUrl, category }) {
  const c = client();
